...
const CouncilDashboard = () => {
  ...
  const validateForm = () => {
    ...
    const docsError = validateFileList(
      form.ownershipDocs,
      true,
      MAX_DOC_SIZE_BYTES,
    );
    ...
    const imagesError = validateFileList(
      form.propertyImages,
      false,
    ...
    try {
      ...
      if (hasFiles) {
        ...
        Object.entries(form).forEach(([key, value]) => {
          if (key === "ownershipDocs" && value) {
            Array.from(value).forEach((file) =>
              payload.append("ownershipDocs", file),
            );
            return;
          }
          if (key === "propertyImages" && value) {
            Array.from(value).forEach((file) =>
              payload.append("propertyImages", file),
            );
            return;
          }
        ...
        const payload = {
          ...form,
          estimatedValue:
            form.estimatedValue === "" ? null : form.estimatedValue,
        };
  ...
  const handleRejectTransfer = async (propertyId) => {
    ...
    try {
      const response = await axios.post(`${BACKEND_URL}/reject-transfer`, {
        propertyId,
      });
      setApprovalStatus({
        type: "success",
